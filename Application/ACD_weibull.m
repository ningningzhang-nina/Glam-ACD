function [parameters, stderrors, LLF, mux,hx,sx,kx, pvalues,AIC,BIC] = ACD_weibull(x,startingvalues,options) 
...
[parameters, LLF, EXITFLAG, OUTPUT, Lambda, GRAD, HESSIAN] =  fmincon('loglikelihood_weibull', startingvalues ,A, b, [],[],lowerbounds, upperbounds, [], options, x);
...
[LLF,likelihoods,mux,hx,sx,kx] = loglikelihood_weibull(parameters, x);
LLF = -LLF;


HESSIAN = zeros(r,r);
epsilon = 0.000000002 * parameters;
for i=1:r
    for j = 1:r
      x1 = parameters;
      x2 = parameters;
      ...
      x4(i) = x4(i) - epsilon(i); 
      x4(j) = x4(j) - epsilon(j);
      [LLF1,~,~] = loglikelihood_weibull(x1, x);
      [LLF2,~,~] = loglikelihood_weibull(x2, x);
      [LLF3,~,~] = loglikelihood_weibull(x3, x);
      [LLF4,~,~] = loglikelihood_weibull(x4, x);
...
for i=1:length(parameters)
    ...
    [holder1,indivlike] =  loglikelihood_weibull(hparameters, x);
