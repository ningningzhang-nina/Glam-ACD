function [parameters, stderrors, LLF, mux, hx, pvalues,AIC,BIC] = ACD_constant_nnsnp1(x,startingvalues,options) 
...
[parameters, LLF, EXITFLAG, OUTPUT, Lambda, GRAD, HESSIAN] =  fmincon('constant_loglikelihood1', startingvalues ,A, b, [],[],lowerbounds, upperbounds, [], options, x);
...
[LLF,likelihoods,mux,hx] = constant_loglikelihood1(parameters, x);
LLF = -LLF;
...
for i=1:r
    for j = 1:r
      ...
      x4(i) = x4(i) - epsilon(i); 
      x4(j) = x4(j) - epsilon(j);
      [LLF1,~,~,~] = constant_loglikelihood1(x1, x);
      [LLF2,~,~, ~] = constant_loglikelihood1(x2, x);
      [LLF3,~,~, ~] = constant_loglikelihood1(x3, x);
      [LLF4,~,~, ~] = constant_loglikelihood1(x4, x);
...
for i=1:length(parameters)
    ...
    [holder1,indivlike] =  constant_loglikelihood1(hparameters, x);
