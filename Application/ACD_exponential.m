function [parameters, stderrors, LLF, mux, hx,sx,kx,pvalues,AIC,BIC] = ACD_exponential(x,startingvalues,options) 
...
[parameters, LLF, EXITFLAG, OUTPUT, Lambda, GRAD, HESSIAN] =  fmincon('loglikelihood_exponential', startingvalues ,A, b, [],[],lowerbounds, upperbounds, [], options, x);
...
[LLF,likelihoods,mux,hx,sx,kx] = loglikelihood_exponential(parameters, x);
LLF = -LLF;
...
for i=1:r
    for j = 1:r
      ...
      x4(i) = x4(i) - epsilon(i); 
      x4(j) = x4(j) - epsilon(j);
      [LLF1,~,~] = loglikelihood_exponential(x1, x);
      [LLF2,~,~] = loglikelihood_exponential(x2, x);
      [LLF3,~,~] = loglikelihood_exponential(x3, x);
      [LLF4,~,~] = loglikelihood_exponential(x4, x);
...
for i=1:length(parameters)
    ...
    [holder1,indivlike] =  loglikelihood_exponential(hparameters, x);
