function [parameters, stderrors, LLF, mux, hx,sx,kx,pvalues,AIC,BIC] = ACD_logexponential(x,startingvalues,type,options) 
% Specify Initial Values
if isempty(startingvalues)
    alpha = 0.0084;
    beta = 0.0405;
    gamma = 0.9511;
    startingvalues = [alpha;beta;gamma];    
 end 
...
[parameters, LLF, EXITFLAG, OUTPUT, Lambda, GRAD, HESSIAN] =  fmincon('loglikelihood_logexponential', startingvalues ,[], [], [],[],[], [], [], options, x,type);
...
[LLF,likelihoods,mux,hx,sx,kx] = loglikelihood_logexponential(parameters, x,type);
LLF = -LLF;


HESSIAN = zeros(r,r);
epsilon = 0.00000004 * parameters;
for i=1:r
    for j = 1:r
      x1 = parameters;
      x2 = parameters;
      ...
      x4(i) = x4(i) - epsilon(i); 
      x4(j) = x4(j) - epsilon(j);
      [LLF1,~,~] = loglikelihood_logexponential(x1, x,type);
      [LLF2,~,~] = loglikelihood_logexponential(x2, x,type);
      [LLF3,~,~] = loglikelihood_logexponential(x3, x,type);
      [LLF4,~,~] = loglikelihood_logexponential(x4, x,type);
...
for i=1:length(parameters)
    ...
    [holder1,indivlike] =  loglikelihood_logexponential(hparameters, x,type);
...
B = strcat(char('alpha','beta','gamma'));
