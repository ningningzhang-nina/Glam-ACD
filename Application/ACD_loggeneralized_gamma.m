function [parameters, stderrors, LLF, mux,hx,sx,kx, pvalues,AIC,BIC] = ACD_loggeneralized_gamma(x,startingvalues,type,options) 
...
if isempty(startingvalues)
    ...
    delta = 1;
    alpha = 1.2789;
    startingvalues = [Alpha;beta;gamma;delta;alpha];    
 end 
...
% upperbounds
lowerbounds  = [-10e8*ones(3,1);1e-8;1];
lowerbounds
upperbounds  = 10e8*ones(5,1);
upperbounds
% define options
if isempty(options)
...
[parameters, LLF, EXITFLAG, OUTPUT, Lambda, GRAD, HESSIAN] =  fmincon('loglikelihood_loggeneralized_gamma', startingvalues ,[], [], [],[],lowerbounds, upperbounds, [], options, x,type);
...
[LLF,likelihoods,mux,hx,sx,kx] = loglikelihood_loggeneralized_gamma(parameters, x,type);
LLF = -LLF;


HESSIAN = zeros(r,r);
epsilon = 0.000000007* parameters;
for i=1:r
    for j = 1:r
      x1 = parameters;
      x2 = parameters;
      ...
      x4(i) = x4(i) - epsilon(i); 
      x4(j) = x4(j) - epsilon(j);
      [LLF1,~,~] = loglikelihood_loggeneralized_gamma(x1, x,type);
      [LLF2,~,~] = loglikelihood_loggeneralized_gamma(x2, x,type);
      [LLF3,~,~] = loglikelihood_loggeneralized_gamma(x3, x,type);
      [LLF4,~,~] = loglikelihood_loggeneralized_gamma(x4, x,type);
...
for i=1:length(parameters)
    ...
    [holder1,indivlike] =  loglikelihood_loggeneralized_gamma(hparameters, x,type);
...
B = strcat(char('Alpha','beta','gamma','delta','alpha'));
