function [parameters, stderrors, LLF, mux,hx,sx,kx, pvalues,AIC,BIC] = ACD_generalized_gamma(x,startingvalues,options) 
...
[parameters, LLF, EXITFLAG, OUTPUT, Lambda, GRAD, HESSIAN] =  fmincon('loglikelihood_generalized_gamma', startingvalues ,A, b, [],[],lowerbounds, upperbounds, [], options, x);
...
[LLF,likelihoods,mux,hx,sx,kx] = loglikelihood_generalized_gamma(parameters, x);
LLF = -LLF;
...
for i=1:r
    for j = 1:r
      ...
      x4(i) = x4(i) - epsilon(i); 
      x4(j) = x4(j) - epsilon(j);
      [LLF1,~,~] = loglikelihood_generalized_gamma(x1, x);
      [LLF2,~,~] = loglikelihood_generalized_gamma(x2, x);
      [LLF3,~,~] = loglikelihood_generalized_gamma(x3, x);
      [LLF4,~,~] = loglikelihood_generalized_gamma(x4, x);
...
for i=1:length(parameters)
    ...
    [holder1,indivlike] =  loglikelihood_generalized_gamma(hparameters, x);
