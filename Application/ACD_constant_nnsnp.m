function [parameters, stderrors, LLF, mux, hx, sx, kx, pvalues,AIC,BIC] = ACD_constant_nnsnp(x,startingvalues,options) 
...
%'myconfun'
[parameters, LLF, EXITFLAG, OUTPUT, Lambda, GRAD, HESSIAN] =  fmincon('constant_loglikelihood2', startingvalues ,A, b, [],[],lowerbounds, upperbounds, [], options, x);
...
[LLF,likelihoods,mux,hx,sx,kx] = constant_loglikelihood2(parameters, x);
LLF = -LLF;
...
for i=1:r
    for j = 1:r
      ...
      x4(i) = x4(i) - epsilon(i); 
      x4(j) = x4(j) - epsilon(j);
      [LLF1,~,~, ~, ~,~] = constant_loglikelihood2(x1, x);
      [LLF2,~,~, ~, ~,~] = constant_loglikelihood2(x2, x);
      [LLF3,~,~, ~, ~,~] = constant_loglikelihood2(x3, x);
      [LLF4,~,~, ~, ~,~] = constant_loglikelihood2(x4, x);
...
for i=1:length(parameters)
    ...
    [holder1,indivlike] =  constant_loglikelihood2(hparameters, x);
