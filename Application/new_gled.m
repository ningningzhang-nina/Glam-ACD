function [llf,loglikehoods,f]=new_gled(params,x)
    alpha=params(1);
    s=params(2);
    k=params(3);
    nu=params(4:end);
...
%     end
%     h=f./(1-F);
%     mu1=compute_noncentral_moments(nu,alpha,1,s,k);
%     mu2=compute_noncentral_moments(nu,alpha,2,s,k);
%     mu3=compute_noncentral_moments(nu,alpha,3,s,k);
%     mu4=compute_noncentral_moments(nu,alpha,4,s,k);
%     mu=mu1;
%     [variance,s,k]=compute_high_moments(mu1,mu2,mu3,mu4);
    loglikehoods=log(f);
    llf=(-sum(log(f)));
end
