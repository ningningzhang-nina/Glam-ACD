function [parameters, stderrors, LLF, mux,hx,sx,kx, pvalues,AIC,BIC] = ACD_logburr(x,startingvalues,type,options) 
...
% upperbounds
lowerbounds  = [-10e8;-10e8;1e-8*ones(2,1)];
lowerbounds
...
[parameters, LLF, EXITFLAG, OUTPUT, Lambda, GRAD, HESSIAN] =  fmincon('loglikelihood_logburr', startingvalues ,A, b, [],[],lowerbounds, upperbounds, [], options, x,type);
...
[LLF,likelihoods,mux,hx,sx,kx] = loglikelihood_logburr(parameters, x,type);
LLF = -LLF;


HESSIAN = zeros(r,r);
epsilon = 0.00000009* parameters;
for i=1:r
    for j = 1:r
      x1 = parameters;
      x2 = parameters;
      ...
      x4(i) = x4(i) - epsilon(i); 
      x4(j) = x4(j) - epsilon(j);
      [LLF1,~,~] = loglikelihood_logburr(x1, x,type);
      [LLF2,~,~] = loglikelihood_logburr(x2, x,type);
      [LLF3,~,~] = loglikelihood_logburr(x3, x,type);
      [LLF4,~,~] = loglikelihood_logburr(x4, x,type);
...
for i=1:length(parameters)
    ...
    [holder1,indivlike] =  loglikelihood_logburr(hparameters, x,type);
