function [parameters, stderrors, LLF, mux, hx, sx, kx, pvalues,AIC,BIC] = ACD_constant_nnsnp4(x,startingvalues,options) 
...
%'myconfun'
[parameters, LLF, EXITFLAG, OUTPUT, Lambda, GRAD, HESSIAN] =  fmincon('constant_loglikelihood4', startingvalues ,A, b, [],[],lowerbounds, upperbounds, [], options, x);
...
[LLF,likelihoods,mux,hx,sx,kx] = constant_loglikelihood4(parameters, x);
LLF = -LLF;


HESSIAN = zeros(r,r);
epsilon = 0.0000001 * parameters;
for i=1:r
    for j = 1:r
      x1 = parameters;
      x2 = parameters;
      ...
      x4(i) = x4(i) - epsilon(i); 
      x4(j) = x4(j) - epsilon(j);
      [LLF1,~,~, ~, ~,~] = constant_loglikelihood4(x1, x);
      [LLF2,~,~, ~, ~,~] = constant_loglikelihood4(x2, x);
      [LLF3,~,~, ~, ~,~] = constant_loglikelihood4(x3, x);
      [LLF4,~,~, ~, ~,~] = constant_loglikelihood4(x4, x);
...
for i=1:length(parameters)
    ...
    [holder1,indivlike] =  constant_loglikelihood4(hparameters, x);
