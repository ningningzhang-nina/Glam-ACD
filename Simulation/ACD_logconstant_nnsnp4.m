function [parameters, stderrors, LLF, mux, hx, sx, kx, pvalues,AIC,BIC] = ACD_logconstant_nnsnp4(x,startingvalues,type,options) 
...
%'myconfun'
[parameters, LLF, EXITFLAG, OUTPUT, Lambda, GRAD, HESSIAN] =  fmincon('logconstant_loglikelihood4', startingvalues ,[],[], [],[],[], [], [], options, x,type);
...
[LLF,likelihoods,mux,hx,sx,kx] = logconstant_loglikelihood4(parameters, x,type);
LLF = -LLF;


HESSIAN = zeros(r,r);
epsilon = 0.00000008 * parameters;
for i=1:r
    for j = 1:r
      x1 = parameters;
      x2 = parameters;
      ...
      x4(i) = x4(i) - epsilon(i); 
      x4(j) = x4(j) - epsilon(j);
      [LLF1,~,~, ~, ~,~] = logconstant_loglikelihood4(x1, x,type);
      [LLF2,~,~, ~, ~,~] = logconstant_loglikelihood4(x2, x,type);
      [LLF3,~,~, ~, ~,~] = logconstant_loglikelihood4(x3, x,type);
      [LLF4,~,~, ~, ~,~] = logconstant_loglikelihood4(x4, x,type);
...
for i=1:length(parameters)
    ...
    [holder1,indivlike] =  logconstant_loglikelihood4(hparameters, x,type);
...
B = strcat(char('alpha','beta','gamma','phi01','phi02','phi03','phi04'));
