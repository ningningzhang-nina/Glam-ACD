function [parameters, stderrors, LLF, mux, hx, sx, kx, pvalues,AIC,BIC] = ACD_logconstant_nnsnp3(x,startingvalues,type,options) 
...
%'myconfun'
[parameters, LLF, EXITFLAG, OUTPUT, Lambda, GRAD, HESSIAN] =  fmincon('logconstant_loglikelihood3', startingvalues ,[],[], [],[],[], [], [], options, x,type);
...
[LLF,likelihoods,mux,hx,sx,kx] = logconstant_loglikelihood3(parameters, x,type);
LLF = -LLF;
...
for i=1:r
    for j = 1:r
      ...
      x4(i) = x4(i) - epsilon(i); 
      x4(j) = x4(j) - epsilon(j);
      [LLF1,~,~, ~, ~,~] = logconstant_loglikelihood3(x1, x,type);
      [LLF2,~,~, ~, ~,~] = logconstant_loglikelihood3(x2, x,type);
      [LLF3,~,~, ~, ~,~] = logconstant_loglikelihood3(x3, x,type);
      [LLF4,~,~, ~, ~,~] = logconstant_loglikelihood3(x4, x,type);
...
for i=1:length(parameters)
    ...
    [holder1,indivlike] =  logconstant_loglikelihood3(hparameters, x,type);
