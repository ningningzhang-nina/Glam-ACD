function [parameters, stderrors, LLF, mux, pvalues,AIC,BIC] = ACD_constant22_nnsnp1(x,startingvalues,options) 
...
if isempty(startingvalues)
    ...
    gamma = 0.9511;
    phi01 = -0.67;
    startingvalues = [alpha;beta;gamma;phi01];    
 end 
...
% upperbounds
lowerbounds  = [1e-8*ones(5,1);-10e8*ones(1,1)];
lowerbounds
upperbounds  = [1;1;1;1;1;10e8*ones(1,1)];
upperbounds
% define options
if isempty(options)
...
[parameters, LLF, EXITFLAG, OUTPUT, Lambda, GRAD, HESSIAN] =  fmincon('constant22_loglikelihood1', startingvalues ,A, b, [],[],lowerbounds, upperbounds, [], options, x);
...
[LLF,likelihoods,mux] = constant22_loglikelihood1(parameters, x);
LLF = -LLF;
...
for i=1:length(parameters)
    ...
    [holder1,indivlike] =  constant22_loglikelihood1(hparameters, x);
...
B = strcat(char('alpha','beta1','gamma1','beta2','gamma2','phi01'));
