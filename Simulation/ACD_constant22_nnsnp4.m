function [parameters, stderrors, LLF, mux,pvalues,AIC,BIC] = ACD_constant22_nnsnp4(x,startingvalues,options) 
...
if isempty(startingvalues)
    ...
    phi03 = 0.05;
    phi04 = 0.05;
    startingvalues = [alpha;beta;gamma;phi01;phi02;phi03;phi04];    
 end 
...
A = [-eye(5) zeros(5,r-5);0 ones(1,4) zeros(1,r-5)]; 
%; 
A
b = [zeros(1,5), 1 - 1e-8];
% 
...
% upperbounds
lowerbounds  = [1e-8*ones(5,1);-10e8*ones(4,1)];
lowerbounds
upperbounds  = [1;1;1;1;1;10e8*ones(4,1)];
upperbounds
% define options
if isempty(options)
...
%'myconfun'
[parameters, LLF, EXITFLAG, OUTPUT, Lambda, GRAD, HESSIAN] =  fmincon('constant22_loglikelihood4', startingvalues ,A, b, [],[],lowerbounds, upperbounds, [], options, x);
...
[LLF,likelihoods,mux] = constant22_loglikelihood4(parameters, x);
LLF = -LLF;


% HESSIAN = zeros(r,r);
% epsilon = 0.00001 * parameters;
% for i=1:r
%     for j = 1:r
%       x1 = parameters;
%       x2 = parameters;
...
%       x4(i) = x4(i) - epsilon(i); 
%       x4(j) = x4(j) - epsilon(j);
%       [LLF1,~,~, ~, ~] = constant_loglikelihood(x1, x);
%       [LLF2,~,~, ~, ~] = constant_loglikelihood(x2, x);
%       [LLF3,~,~, ~, ~] = constant_loglikelihood(x3, x);
%       [LLF4,~,~, ~, ~] = constant_loglikelihood(x4, x);
...
for i=1:length(parameters)
    ...
    [holder1,indivlike] =  constant22_loglikelihood4(hparameters, x);
...
B = strcat(char('alpha','beta1','gamma1','beta2','gamma2','phi01','phi02','phi03','phi04'));
