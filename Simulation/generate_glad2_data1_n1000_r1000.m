...
syms x f(x) z(x)
% initiate parameters
replication_time=1000;
num=1000;
alpha=0;
s=1;
nu=[-0.5188,-0.2702];
k=(1+nu(1)^2+nu(2)^2)/(1-2*nu(1)+2*nu(1)^2-4*nu(1)*nu(2)+4*nu(2)^2);
...
%%
%count=0;
for repeat=265:replication_time
    tic;
    ite=0;
    while ite<=num
        count=count+1;
...
save("simulation_glgd2_data1_n1000_r1000.mat")
